    ...
    $(document).ready(function() {
            ...
            courses.forEach(function(course) {
                courseInput.append($('<option>', {
                    value: course.subject._id,
                    text: course.subject.name
                }));
            });
    
            // Function to update fields based on selected course
            function updateFields(courseId) {
                var selectedCourse = courses.find(function(course) {
                    return course.subject._id === courseId;
                });
    
                $('#creditInput').val(selectedCourse.subject.creditUnits);
                $('#theoryHoursInput').val(selectedCourse.subject.theorySessions);
                $('#practiceHoursInput').val(selectedCourse.subject.practicalSessions);
            }
    
            // Update fields when courseInput changes
            courseInput.on('change', function() {
                var selectedCourseId = $(this).val();
                updateFields(selectedCourseId);
            });
    
            // Initialize fields based on default selected course
            var defaultCourseId = courseInput.val();
            updateFields(defaultCourseId);
        });
    function displayCourseList(courses) {
        ...
        courses.forEach(function (course, index) {
           ...
           row.querySelector('.edit-button').addEventListener('click', function () {
            ...
            document.getElementById('courseInput').value = course.subject._id;
    ...
    document.getElementById("submitBtn").addEventListener("click", function(event) {
        ...
        var theoryHoursInput = document.getElementById("theoryHoursInput");
        var practiceHoursInput = document.getElementById("practiceHoursInput");
        var feeInput = document.getElementById("feeInput");
        var semesterSelect = document.getElementById("semesterSelect");
        var selectedSemesterValue = semesterSelect.value;
        var selectedSemester = semesters.find(function(semester) {
            return semester._id === selectedSemesterValue;
    });
        var statusRadio = document.querySelector('input[name="statusRadio"]:checked');
    
        // Tạo một đối tượng mới đại diện cho một khóa học từ dữ liệu đã lấy được
        var newCourse = {
            "_id": "", // Để tạo một ID mới cho khóa học
            "subject": {
                "_id": generateUUID(),
                "name": courseInput.options[courseInput.selectedIndex].text,
                "creditUnits": parseInt(creditInput.value),
                "theorySessions": parseInt(theoryHoursInput.value),
                "practicalSessions": parseInt(practiceHoursInput.value)
            },
            "tuitionFee": parseInt(feeInput.value),
            "semester": selectedSemester,
            "status": statusRadio.value,
            "_class": "edu.iuh.administratorservice.entity.Course"
        };
         
        courses.push(newCourse);
        alert('Thêm lớp học phần thành công!');
        // Thêm đối tượng khóa học mới vào mảng courses
        displayCourseList(courses);
        document.getElementById("feeInput").value="";
        document.querySelectorAll('input[name="statusRadio"]').forEach(function(radio) {
            radio.checked = false;
        });
    ...
    document.getElementById("updateBtn").addEventListener("click", function(event) {
        ...
        var statusRadio = document.querySelector('input[name="statusRadio"]:checked');
        // Tìm môn học tương ứng trong mảng courses
        var updatedCourseIndex = courses.findIndex(function(course) {
        ...
        if (updatedCourseIndex !== -1) {
            ...
            courses[updatedCourseIndex].subject.name = courseInput.options[courseInput.selectedIndex].text;
            courses[updatedCourseIndex].subject.creditUnits = parseInt(creditInput.value);
            courses[updatedCourseIndex].subject.theorySessions = parseInt(theoryHoursInput.value);
            courses[updatedCourseIndex].subject.practicalSessions = parseInt(practiceHoursInput.value);
            courses[updatedCourseIndex].tuitionFee = parseInt(feeInput.value);
            courses[updatedCourseIndex].semester = selectedSemester;
            courses[updatedCourseIndex].status = statusRadio ? statusRadio.value : ""; // Kiểm tra xem có radio button nào được chọn không
            ...
            document.getElementById("feeInput").value = "";
            // Đặt lại giá trị của radio button
            document.querySelectorAll('input[name="statusRadio"]').forEach(function(radio) {
                radio.checked = false;
            });
        } else {
            alert('Không tìm thấy môn học để cập nhật!');
        }
    });
